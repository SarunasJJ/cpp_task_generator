...
DESCRIPTION:
[INTRODUCTION - 2 to 4 sentences that explain the concept or provide a motivating
...
[INSTRUCTIONS - A precise paragraph stating exactly what the program must do.
...
[WORKED EXAMPLE - Show at least one concrete example. Give the specific input
values, trace through the relevant intermediate steps (not just input and output),
and state the expected output. Use real numbers, not placeholders.]
...
REQUIREMENTS:
1. DESCRIPTION — Introduction section: explain the concept fully. Never assume the student \
knows a named algorithm (e.g. Euclidean algorithm, Fibonacci) — define it before asking \
them to implement it.
2. DESCRIPTION — Instructions section: specify stdin format (data types, number of values, \
...
3. DESCRIPTION — Worked example: show concrete input values, relevant intermediate steps, \
and the expected output. Do not use generic placeholders.
4. SOLUTION: must be a complete, compilable C program with main() that reads from stdin \
and prints to stdout. Include all necessary #include lines.
5. TEST_CASES: expected outputs must be character-perfect, including spacing and newlines. \
Cover a typical case, a boundary/edge case, and one additional case.
6. The description and test cases must be consistent — the worked example in the description \
